...
export default function Prods(props) {
  ...
  const onAdd=(product)=>{
      AddtoCart(product)
      toast({
        title: `Item ${product.name} Added to Cart !`,
        status: 'success',
        duration: 2000,
      })
  }
          ...
          />
            ...
            <Flex mt="1" justifyContent="space-between">
              <Box
                ...
                lineHeight="tight"
                isTruncated>
                <Heading as="h5" size='sm'>{ product.name} </Heading>
                <Heading as="h5" size='sm'>{ product.shop} </Heading>
              </Box>
              <Tooltip
                label="Add to cart"
                bg="white"
